#### 컴포넌트를 통해 해결하고 싶은 문제
- 뷰와 데이터 로직의 확장
- 부모-자식간의 명확한 인터페이스 분리

- 추론이 쉽고 유지 관리가 쉬운 의사소통 방법, 잠재적으로 쉽게 `재사용`
- [부모-자식간의 명확한 인터페이스 분리로 격리(캡슐화)](컴포넌트-작성)
- [콘텐츠 배포 프로세스](#콘텐츠-배포-프로세스)로 뷰와 데이터 로직의 확장
- [자식 컴포넌트 참조](#자식-컴포넌트-참조)로 로직 분업화
- 단순한 인터페이스의 단순화 : Vue 인스턴스를 사용하여 `모든 옵션 객체`, `라이프 사이클 훅`를 사용할 수 있다.
