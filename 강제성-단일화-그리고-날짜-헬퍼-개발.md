...
### 도구로 해결할 수 없는 강제성
우선 강제성의 사전적 정의는 "본인의 의사와는 관계없이 권력이나 힘을 이용해 원하지 않는 일을 억지로 시키는 경향" 이다.

프로젝트에서는 코딩 컨벤션과 유닛 테스트 통과라는 강제성을 부여하기도 했다. 코드리뷰를 통한 사람대 사람으로 피드백을 주는 방식은 감정을 상할 여지가 있다. 때문에 도구를 통해 프로젝트의 그라운드 룰을 강제하는 것이다.

하지만 도구로 해결할 수 없는 것이 있다. 그것은 코드의 동작을 컨트롤 하는 것이다. 예를 들어 날짜를 의미하는 값은 다양한 방법이 있다.
...
### 날짜 헬퍼를 만들게 된 이유
우선 자바스크립트에서 날짜를 다루는 API는 **Date**를 사용한다. **Date**를 사용해서 달력이나 지난시간, D-Day를 계산하게 된다. 날짜 헬퍼는 이런 계산작업을 한 곳에서 정의하기 위한 공통 코드를 의미한다.

이제 왜 날짜 헬퍼를 만들게 되었는 지와 왜 필요한지 알아보겠다.

날짜를 다루는 타입은 4가지가 있었다. 
1. 숫자타입의 타임스탬프
2. YYYY-MM-DD 형태의 문자타입
3. Moment 라이브러리
4. Date

하지만 계산 작업은 많은 타입이 필요하지 않다. YYYY-MM-DD의 형태는 API 통신이나 탬플릿에 필요한 타입이다. 타임스탬프는 비교 연산을 위해 사용되는 데, 자바스크립트는 문자도 비교연산을 사용가능하다. 즉, 타임스탬프도 불필요하다.
...
```
날짜를 의미하는 타입이 불규칙적이다. 불규칙적이기 때문에 코드를 해석하기 힘들어 졌다. 그래서 단일 타입을 강제하는 게 필요해졌다.

단일 타입과 강제성을 부여하기 위해서는 한 위치에서 관리하는 게 좋다. 새로운 날짜값을 만들거나 계산이 필요할 때 헬퍼를 통해서 한다. 사용측에서는 "어떤 타입을 사용해야 하는지", "어떻게 연산해야 하는 지" 관리나 고민할 필요가 없어진다.
